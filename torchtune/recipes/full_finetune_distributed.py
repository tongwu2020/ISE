...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def _setup_optimizer(
        ...
        seg_lr = cfg_optimizer.get("seg_lr_scale", False)
        
        if seg_lr is not False:
            param_groups = [
            {
                'params': [param for name, param in self._model.named_parameters() if 'seg_embeddings' in name],
                'lr': cfg_optimizer.lr * cfg_optimizer.seg_lr_scale
            },
            {
                'params': [param for name, param in self._model.named_parameters() if 'seg_embeddings' not in name],
                'lr': cfg_optimizer.lr 
            }
            ]

            # remove the seg_lr_scale from the optimizer config
            optimizer_name = cfg_optimizer.pop("_component_")
            assert optimizer_name == "torch.optim.AdamW", "Only AdamW optimizer is supported for segment learning rate"
    ...
    def train(self) -> None:
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            ...
            for idx, batch in enumerate(self._dataloader):
                ...
                input_seg = batch.get("segment", None)  # shape [b, s]
                ...
                if input_seg is not None:
                    logits = self._model(tokens, mask=mask, input_pos=input_pos, input_seg=input_seg)
                else:
                ...
                # ## testing the model parameters updates of segment
                # for name, param in self._model.named_parameters():
                #     if "seg_embeddings" in name:
                #         print("parameter", param)

                # Step with optimizer
                if (idx + 1) % self._gradient_accumulation_steps == 0:
                    self._optimizer.step()
                    ## print the graident of the segment embeddings
                    # for name, param in self._model.named_parameters():
                    #     if "seg_embeddings" in name:
                    ...
                    if (
                        ...
                        log_dict = {
                            "total_steps_per_epoch": self._steps_per_epoch,
                            "loss": loss_to_log,

...
def lora_llama3_segment(
    lora_attn_modules: List[LORA_ATTN_MODULES],
    apply_lora_to_mlp: bool = False,
    apply_lora_to_output: bool = False,
    *,
    # llama3 args
    segment_size: int, # number of segments
    vocab_size: int,
    num_layers: int,
    num_heads: int,
    num_kv_heads: int,
...
def llama3_segment(
    ...
    tok_embeddings = nn.Embedding(vocab_size, embed_dim)
    seg_embeddings = nn.Embedding(segment_size, embed_dim) # 4 segments: sys, user, data, assistant
    nn.init.normal_(seg_embeddings.weight, mean=0, std=0.01) # Weight initialization can be tuned, we use 0.01 or 0.02 for llama3_8B
    output_proj = nn.Linear(embed_dim, vocab_size, bias=False)

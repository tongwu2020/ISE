...
def padded_collate(
    ...
    segment = batch[0].get("segment", None)
    if segment is not None:
        segment = pad_sequence(
            [torch.tensor(x["segment"]) for x in batch],
            batch_first=True,
            padding_value= 3,  # padding with assiant segment id
        )
        if input_ids_seq_len > labels_seq_len:
            segment = F.pad(
                segment, (0, input_ids_seq_len - labels_seq_len), value= 3 # padding with assiant segment id
        )
        return {"tokens": input_ids, "labels": labels, "segment": segment}
...
def padded_collate_left(
    ...
    """
    max_input_len = max(len(x["tokens"]) for x in batch)
    max_label_len = max(len(x["labels"]) for x in batch)

    input_ids = [
        torch.tensor(
            [padding_idx] * (max_input_len - len(x["tokens"])) + x["tokens"]
        )
        for x in batch
    ]
    ...
    labels = [
        torch.tensor(
            [ignore_idx] * (max_label_len - len(x["labels"])) + x["labels"]
        )
        for x in batch
    ]
    ...
    segment = batch[0].get("segment", None)
    if segment is not None:
        max_segment_len = max(len(x["segment"]) for x in batch)
        segment = [
            torch.tensor(
                [3] * (max_segment_len - len(x["segment"])) + x["segment"]
            )
            for x in batch
        ]

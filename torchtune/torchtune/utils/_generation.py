...
def generate_next_token_seg(
    ...
    # model produces logits in [bsz, seq_length, vocab_size]
    # we want to take the last token's logits as the input to the next model call
    # print("input_pos", input_seg)
    logits = model(x, input_pos=input_pos, input_seg= input_seg)[:, -1]
    return sample(logits, temperature, top_k)
...
def generate_segment(
    ...
    *,
    ...
    if custom_generate_next_token is None:
        custom_generate_next_token = generate_next_token_seg

    # generate the first tokens conditioned on the prompt
    input_pos = torch.arange(0, model.max_seq_len, device=prompt.device)
    tokens = generate_next_token_seg(
        model,
        input_pos=input_pos[:prompt_length],
        input_seg=input_seg[:prompt_length],
        x=prompt,
        temperature=temperature,
        top_k=top_k,
    )
    ...
    for _ in range(max_generated_tokens - 1):
        ...
        # otherwise, we take the whole sequence up to the current position
        if incremental_decoding:
            #print("incremental_decoding")
            curr_input_pos = input_pos[curr_pos].unsqueeze(0)
            curr_input_seg = input_seg[0][-1].unsqueeze(0)
        else:
            curr_input_pos = input_pos[: curr_pos + 1]
            # curr_input_seg should add the last token of the input_seg
            curr_input_seg = torch.zeros((1, curr_pos + 1), dtype=torch.int32, device=prompt.device) + 3
            curr_input_seg[0][:len(input_seg[0])] = input_seg[0]
            tokens = generated_tokens.clone()
            # print("curr_input_pos", curr_input_pos, "curr_input_seg", curr_input_seg)

        tokens = custom_generate_next_token(
            model,
            input_pos=curr_input_pos,
            input_seg=curr_input_seg,
            x=tokens,
            temperature=temperature,
            top_k=top_k,
        )
